...
int INIT_openssl(void)
{
    ...
    SSL_load_error_strings();               // すべてのlibssl、libcrypto関数のエラー文字列を登録します。(メモリが心配なら不要。OpenSSL 1.1.0以降非推奨 → OPENSSL_init_ssl()およびOPENSSL_init_crypto()へ)
    SSL_library_init();                     // 利用可能なSSL/TLS暗号とダイジェストを登録します。(OpenSSL 1.1.0以降非推奨 → OPENSSL_init_ssl()へ)
    OpenSSL_add_all_algorithms();           // すべてのアルゴリズム（ダイジェストと暗号）をテーブルに追加します。(OpenSSL 1.1.0以降非推奨 → OPENSSL_init_crypto()へ)

    // ----------------
    // SSL設定情報を作成 (サーバー用のTLSメソッドを指定、1.1.0以降はTLS_server_method()を指定すること。SSL_CTX_set_options()でいずれにしても許可するプロトコルバージョンを指定すること)
    // ----------------
...
int INIT_socket(struct EVS_port_t *listen_port)
{
    ...
    if (listen_port->port == 0)
    {
        ...
        // ----------------
        server_watcher = (struct EVS_ev_server_t *)calloc(1, sizeof(*server_watcher));
        // メモリ領域が確保できなかったら
...
int INIT_all(int argc, char *argv[])
{
    ...
    // タイマーイベント初期化処理
    // --------------------------------
    // タイマーオブジェクトに対して、コールバック処理とタイムアウト確認間隔(timeout_checkintval秒)、そして繰り返し回数(0回)を設定する
    ev_timer_init(&timeout_watcher, CB_timeout, EVS_config.timeout_checkintval, 0);
